    ...
    export default {
        ...
        mounted(){
             api.PLUGIN_API().then(res => {
                this.menuList = []
                var kind = { id: '1', name: '插件模板',open: true}

                var children = []
                for (var i = 0; i < res.length; i++){
                    var resource = {icon:"el-icon-setting",id: "1" + i, name:res[i].name, annoName:res[i].annoName, configUrl:res[i].configUrl?res[i].configUrl : "", resultUrl:res[i].resultUrl?res[i].resultUrl:"" }
                    children.push(resource)
                }
                kind["children"] = children
                this.menuList.push(kind)
            })
        },
        created() {
            /**
