...
export default {
  ...
  methods: {
    ...
    deleteItem(item, index) {
      //调用api 删除数据库中构建记录
      this.$confirm("确定要删除构建 " + item.no + "？", "提示", {
    ...
    loadHistory() {
      ...
      if (that.$route.params.pipelineId) {
        api
          .PIPELINE_BUILD_HISTORY_API(that.$route.params.pipelineId)
          .then((res) => {
            res.forEach((item, index) => {
              ...
              let runningPipeline = {
                ...
                buildTime: item.startTime,
                status: pipelineStatus,
              };
              if (that.listData.length <= index) {
                that.listData.unshift(runningPipeline);
              } else {
                that.listData[that.listData.length - 1 - index].percent =
                  item.percent;
                that.listData[
                  that.listData.length - 1 - index
                ].status = pipelineStatus;
                that.listData[that.listData.length - 1 - index].buildTime =
                  item.startTime;
              }
            });
